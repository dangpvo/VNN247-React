export const rssMapHome = {
  latestNews: [
    { source: "VnExpress", url: "https://vnexpress.net/rss/tin-moi-nhat.rss" },
  ],
  hotNews: [
    { source: "VnExpress", url: "https://vnexpress.net/rss/tin-noi-bat.rss" },
    { source: "DanTri", url: "https://dantri.com.vn/rss/tam-diem.rss" },
  ],
  economy: [
    { source: "VnExpress", url: "https://vnexpress.net/rss/kinh-doanh.rss" },
    { source: "ThanhNien", url: "https://thanhnien.vn/rss/thi-truong.rss" },
    { source: "ThanhNien", url: "https://thanhnien.vn/rss/kinh-te.rss" },
    { source: "TuoiTre", url: "https://tuoitre.vn/rss/kinh-doanh.rss" },
    { source: "DanTri", url: "https://dantri.com.vn/rss/kinh-doanh.rss" },
    { source: "DanTri", url: "https://dantri.com.vn/rss/bat-dong-san.rss" },
    { source: "PhapLuat", url: "https://plo.vn/rss/kinh-te-13.rss" },
  ],
  education: [
    { source: "VnExpress", url: "https://vnexpress.net/rss/giao-duc.rss" },
    { source: "ThanhNien", url: "https://thanhnien.vn/rss/giao-duc.rss" },
    { source: "TuoiTre", url: "https://tuoitre.vn/rss/giao-duc.rss" },
    { source: "DanTri", url: "https://dantri.com.vn/rss/giao-duc.rss" },
    { source: "PhapLuat", url: "https://plo.vn/rss/giao-duc-21.rss" },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/giao-duc/chon-truong-chon-nghe-313.rss",
    },
  ],
  articles: [
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/blog-phong-vien.rss",
    },
    { source: "ThanhNien", url: "https://thanhnien.vn/rss/toi-viet.rss" },
    { source: "DanTri", url: "https://dantri.com.vn/rss/photo-news.rss" },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/thoi-su-/chinh-kien-250.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/thoi-su/theo-dong-thoi-su-4.rss",
    },
  ],
};

export const rssMapArticles = {
  culture: [
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/van-hoa.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/van-hoa.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/xa-hoi-3.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/van-hoa-16.rss",
    },
  ],
  iWrite: [
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/toi-viet.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/ban-doc.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/blog-phong-vien.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/ban-doc.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/ban-doc.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/ban-doc-24.rss",
    },
  ],
  photoStory: [
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/photo-story.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/tam-long-nhan-ai.rss",
    },
  ],
  timeEssay: [
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/thoi-su/theo-dong-thoi-su-4.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/thoi-su-/chinh-kien-250.rss",
    },
  ],
  criminalRecord: [
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/an-ninh-trat-tu/ho-so-pha-an-166.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/an-ninh-trat-tu/truy-na-201.rss",
    },
  ],
};

export const rssMapNews = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/thoi-su.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/thoi-su.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/thoi-su.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/su-kien.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/quoc-te/su-kien-223.rss",
    },
  ],
};

export const rssMapWorld = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/the-gioi.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/the-gioi.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/the-gioi.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/the-gioi.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/quoc-te-8.rss",
    },
  ],
};

export const rssMapEconomy = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/kinh-doanh.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/kinh-te.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/thi-truong.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/kinh-doanh.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/kinh-doanh.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/bat-dong-san.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/kinh-te-13.rss",
    },
  ],
};

export const rssMapEducation = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/giao-duc.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/giao-duc.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/giao-duc.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/giao-duc.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/giao-duc-21.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/giao-duc/chon-truong-chon-nghe-313.rss",
    },
  ],
};

export const rssMapHealth = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/suc-khoe.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/suc-khoe.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/suc-khoe.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/suc-khoe.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/suc-khoe-17.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/suc-khoe/bac-si-online-285.rss",
    },
  ],
};

export const rssMapEntertainment = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/giai-tri.rss",
    },
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/cuoi.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/giai-tri.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/gioi-tre.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/thoi-trang-tre.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/giai-tri.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/thu-gian.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/nhip-song-tre.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/giai-tri.rss",
    },
  ],
};

export const rssMapJobs = {
  main: [
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/viec-lam.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/lao-dong-viec-lam.rss",
    },
  ],
};

export const rssMapLaws = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/phap-luat.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/chinh-tri.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/phap-luat.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/phap-luat.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/thoi-su/chinh-tri-2.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/phap-luat-114.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/phap-luat/chinh-sach-moi-116.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/phap-luat/luat-va-doi-213.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/kinh-te/phap-ly-4-0-286.rss",
    },
  ],
};

export const rssMapSports = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/the-thao.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/the-thao.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/the-thao.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/the-thao.rss",
    },
    {
      source: "PhapLuat",
      url: "https://plo.vn/rss/the-thao-22.rss",
    },
  ],
};

export const rssMapLife = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/gia-dinh.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/doi-song.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/tieu-dung-thong-minh.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/gia-that.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/doi-song.rss",
    },
  ],
};

export const rssMapTravel = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/du-lich.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/du-lich.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/du-lich.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/du-lich.rss",
    },
  ],
};

export const rssMapScience = {
  main: [
    {
      source: "VnExpress",
      url: "https://vnexpress.net/rss/khoa-hoc-cong-nghe.rss",
    },
    {
      source: "ThanhNien",
      url: "https://thanhnien.vn/rss/cong-nghe.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/khoa-hoc.rss",
    },
    {
      source: "TuoiTre",
      url: "https://tuoitre.vn/rss/nhip-song-so.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/cong-nghe.rss",
    },
    {
      source: "DanTri",
      url: "https://dantri.com.vn/rss/khoa-hoc.rss",
    },
  ],
};
